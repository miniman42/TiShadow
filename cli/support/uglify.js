...
function addAppName(node) {
  if (config.isHideShadow) {
    return new UglifyJS.AST_Binary({
      left: node,
      operator: "+",
      right: new UglifyJS.AST_Symbol({
        name: 'require("/api/CarmaOne").currentApp + "/"'
      })
    });
  } else {
    return new UglifyJS.AST_Binary({
      ...
      operator: "+",
      right: new UglifyJS.AST_Symbol({
        name: 'require("/api/TiShadow").currentApp + "/"'
      })
    });
  }
}
...
var convert = new UglifyJS.TreeTransformer(null, function(node) {
  ...
  if (node instanceof UglifyJS.AST_Call) {
    ...
    //assets
    if (node.expression.end.value.match("^set") && !doNotTouch(node.args) &&
      couldBeAsset(node.expression.end.value.replace("set", "").toLowerCase())) {
      node.args[0].value = toFullPath(node.args[0].value);
      //console.log(current_file + ' (node.args) About to assign:' + node.args[0].value);
    ...
    if (node.left.property && node.left.property.match && node.left.property.match("^(title|text)id$")) {
      ...
      //console.log(current_file + '(right.value) About to assign: ' + node.right.value);
...
exports.toString = function(input, file) {
  return exports.toAST(input, file).print_to_string({
    beautify: true
  });
};